...
c3.metric("매칭된 후보", status["matched_candidates"])
c4.metric("미매칭 후보", status["unmatched_candidates"])
...
if status["children"]:
    ...
    st.caption("⚠️ 표시는 권장 최소치 미만(부족) 셀입니다.")
else:
    st.info(
        "유아별 행이 없습니다. 후보가 가명 ID로 **매칭**되어야 유아별 현황이 표시됩니다. "
        "(매칭은 ‘주간 관찰초안’ 화면에서 교사가 확정합니다.)"
    )

# 미매칭 집계 표시
if status["unmatched"]["total"] > 0:
    with st.expander(f"미매칭 후보 집계 (총 {status['unmatched']['total']}건)", expanded=False):
        um = status["unmatched"]["cells"]
        st.dataframe(
            [{"누리영역": a, "수집 건수": um[a]["count"]} for a in NURI_AREAS],
            use_container_width=True, hide_index=True,
        )
        st.caption("아직 가명 ID와 매칭되지 않은 후보입니다. 매칭 후 유아별 현황에 반영됩니다.")
